...
results_files = "results_demo_cls_ensemble.txt"
# clean previous results
if os.path.isfile(results_files):
    os.remove(results_files)

# estim_algorithm = "moval-ensemble-debug"
# mode = "classification"
# metric = "auc"
@pytest.mark.parametrize(
        "estim_algorithm, mode, metric", 
        list(itertools.product(["moval-ensemble-debug"],
                               ["classification"],
                               ["accuracy", "sensitivity", "precision", "f1score", "auc"])),
)
def test_cls(estim_algorithm, mode, metric):
    ...
    with open(results_files, 'a') as f:
        ...
        f.write(str(err_val))
        f.write('\n')
        f.write("moval ensembling conditions: ")
        f.write(str(moval_model.ensemble_conds))
        f.write('\n')
        for model in moval_model.model_:
            f.write("moval parameter: ")
            f.write(str(model.param))
            f.write('\n')
            if model.extend_param:
                f.write("moval extended parameter: ")
                f.write(str(model.param_ext))
                f.write('\n')
        f.write("real metric: ")
